...
totalTime=199.97;      %duration of experiment
...
for simTime=0:dt:totalTime
    ...
    mdot_liquid=volumetric_flow_rate(count)*rhol;       %from tank pressure
    m_liquid=(mdot_liquid*dt)+m_liquid;
    
    %mass of water vapor in CC [kg]
    m_vapor=HerKnu(Pv,dT,Pg)+m_vapor;
    %-mDotThruOrifice(Pg,0,fluid_density,1.33,0.01,0.0001))*dt)
...
figure(1)
plot((0:dt:totalTime),CCPressure(1:length(CCPressure)-1),'Linewidth',3)
hold on
